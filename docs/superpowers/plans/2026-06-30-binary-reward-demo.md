...
{ "compilerOptions": { "module": "commonjs", "target": "es2021",
  ...
  "esModuleInterop": true, "outDir": "./dist", "baseUrl": "./src" } }
```
`nest-cli.json`:
```json
...
```http
### create root
POST http://localhost:3001/tree/root
Content-Type: application/json
{ "username": "root", "password": "p" }

### register a under root-L
POST http://localhost:3001/tree/register
Content-Type: application/json
...
POST http://localhost:3001/settlement/run

### check root user wallet (expect walletBalance 350 after worker runs)
# GET http://localhost:3001/users/<rootUserId>
```
